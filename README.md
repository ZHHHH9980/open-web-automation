...
如果你想**强制测试 OpenAI planner**（不走默认的 Claude / auto 选择），可以直接在仓库根目录运行：

```bash
export OPENAI_API_KEY=你的key
./test.sh "在知乎搜索 AI Agent，整理前 3 条结果"
```
...
- `automation-web/docs/EXECUTION_CONTRACT.md:1`：执行输入输出与状态语义
- `automation-web/docs/RUNBOOK.md:1`：启动、验证与排障步骤
