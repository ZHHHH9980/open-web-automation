...
function isXiaohongshuSearchResult(state) {
...
async function executeXiaohongshuClick(page, action, state, context = {}) {
  ...
  const visitedArticleKeys = Array.isArray(siteState.visitedArticleKeys)
    ? siteState.visitedArticleKeys
    : [];
...
async function executePlatformClick(page, action, state, context = {}) {
  if (isXiaohongshuSearchResult(state)) {
    return executeXiaohongshuClick(page, action, state, context);
  }
